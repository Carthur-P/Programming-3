...
public class ArrayAndListUtility {
	...
	public static void bubbleSortFirend(Friend[] array)
	{
		boolean sorted = false;
		do 
		{
			...
			for (int j = 0; j < array.length - 1; j++)
			{
				if (array[j].getLast().compareTo(array[j + 1].getLast()) > 0)
				{
					Friend temp = array[j];
					array[j] = array[j + 1];
					array[j + 1] = temp;
	...
	public static String binarySearch(int[] array, int find)
	{
		bucketSort(array);
		int left = 0;
		...
		while (left <= right)
		{
			...
			{
				result = Integer.toString(middle);
				return result;
			}
		...
		return "search could not find the number";
	} 
	
	public static String binarySearchString(String[] array, String find)
	{
		...
		{
			int middle = (left + right)/2;
			if (array[middle].equals(find))
			{
				result = "word found at position " + Integer.toString(middle);
				return result;
			}
		...
		return "search could not find the word";
	} 
}
